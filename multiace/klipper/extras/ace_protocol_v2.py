...
SLOT_STATES = {0: 'ready', 1: 'feeding', 2: 'rollback', 3: 'assisting', 4: 'rollback_assisting', 5: 'preloading', 6: 'upgrading', 129: 'feed_error', 130: 'rollback_error', 131: 'assist_error', 132: 'preload_error', 133: 'stuck_error', 134: 'tangled_error', 135: 'motor_error'}
FILAMENT_STATES = {0: 'empty', 1: 'unknown', 2: 'identified', 3: 'identifying'}
DRY_STATES = {0: 'stop', 1: 'starting', 2: 'keeping', 3: 'stopping', 4: 'ptc_error', 5: 'ntc_error'}
...
def crc16_kermit(data):
    ...
    for byte in data:
        crc ^= byte
        for _ in range(8):
            crc = crc >> 1 ^ 33800 if crc & 1 else crc >> 1
    return crc & 65535

def pb_varint(value):
    r = bytearray()
    while value > 127:
        r.append(value & 127 | 128)
        value >>= 7
    r.append(value & 127)
    return bytes(r)
...
DEFAULT_DAEMON_SOCKET = '/tmp/multiace_v2.sock'

class SocketTransport(object):

    def __init__(self, sock_path, read_timeout=0.1):
    ...
    def write(self, data):
        if self._closed:
            raise IOError('SocketTransport closed')
...
class AceProtocolV2(AceProtocol):
    ...
    DAEMON_BINARY = '/usr/local/bin/multiace_v2d.py'
    DAEMON_PYTHON = '/usr/bin/python3'
    DAEMON_SPAWN_TIMEOUT = 3.0
    ...
    def _ensure_daemon(cls):
        if os.path.exists(cls.DAEMON_SOCKET):
            return True
        if not os.path.exists(cls.DAEMON_BINARY):
            logging.info('[multiACE] V2 daemon binary missing: %s', cls.DAEMON_BINARY)
            return False
        v2_paths = cls._scan_v2_serial_paths()
        if not v2_paths:
            return False
        v2_port = v2_paths[0]
        import subprocess
        ...
        try:
            subprocess.Popen([cls.DAEMON_PYTHON, cls.DAEMON_BINARY, '--port', v2_port, '--socket', cls.DAEMON_SOCKET, '--baud', str(cls.DEFAULT_BAUD)], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, stdin=subprocess.DEVNULL, close_fds=True, start_new_session=True)
        except Exception as e:
            logging.info('[multiACE] V2 daemon spawn failed: %s', e)
            return False
        logging.info('[multiACE] V2 daemon spawned for %s', v2_port)
        deadline = _time.monotonic() + cls.DAEMON_SPAWN_TIMEOUT
        while _time.monotonic() < deadline:
            if os.path.exists(cls.DAEMON_SOCKET):
                return True
            _time.sleep(0.1)
        logging.info('[multiACE] V2 daemon socket did not appear within %.1fs', cls.DAEMON_SPAWN_TIMEOUT)
        return False
    ...
    def encode_request(self, request, next_id=None):
        seq = int(request.get('id', 1)) & 65535
        method = request.get('method', '')
        params = request.get('params', {}) or {}
        cmd, payload = self._method_to_v2(method, params)
        if len(payload) > MAX_PAYLOAD_LEN:
            raise ValueError('V2 payload exceeds %d bytes for method %s' % (MAX_PAYLOAD_LEN, method))
        inner = bytearray([FLAG_REQUEST, seq & 255, seq >> 8 & 255, cmd & 255, len(payload) & 255])
        inner.extend(payload)
        crc = crc16_kermit(bytes(inner))
        return bytes(PREAMBLE) + bytes(inner) + bytes([crc & 255, crc >> 8 & 255, END_MARKER])
    ...
    def decode_frames(self, buffer):
        ...
        while len(buffer) >= MIN_FRAME_LEN:
            ...
            if crc_in_frame != crc_calc:
                logging.info('[multiACE] V2 CRC mismatch (calc=%04x frame=%04x), dropping frame', crc_calc, crc_in_frame)
    ...
    def initial_handshake_requests(self):
        return [{'method': 'discover_device'}, {'method': 'get_info'}]

    def make_default_info(self):
        return {'status': 'ready', 'dryer_status': {'status': 'stop', 'target_temp': 0, 'duration': 0, 'remain_time': 0}, 'temp': 0, 'enable_rfid': 1, 'fan_speed': 7000, 'feed_assist_count': 0, 'cont_assist_time': 0.0, 'slots': [{'index': i, 'status': 'empty', 'sku': '', 'type': '', 'rfid': 0, 'brand': '', 'color': [0, 0, 0]} for i in range(4)]}
    ...
    def _method_to_v2(self, method, params):
        ...
        if method == 'start_feed_assist':
            ...
            payload = pb_uint32(1, slot) + pb_uint32(2, speed) + pb_uint32(3, 0) + pb_uint32(4, FEED_MODE_ASSIST)
        ...
        if method == 'feed_filament':
            ...
            payload = pb_uint32(1, slot) + pb_uint32(2, speed) + pb_uint32(3, length) + pb_uint32(4, FEED_MODE_FEED)
            return (Cmd.FEED_OR_ROLLBACK, payload)
        if method == 'unwind_filament':
            slot = int(params.get('index', 0))
            length = int(params.get('length', 0))
            speed = int(params.get('speed', 50))
            payload = pb_uint32(1, slot) + pb_uint32(2, speed) + pb_uint32(3, length) + pb_uint32(4, FEED_MODE_ROLLBACK)
            return (Cmd.FEED_OR_ROLLBACK, payload)
        ...
        if method == 'set_fan_speed':
            ...
            payload = pb_uint32(1, speed) + pb_bool(2, speed > 0) + pb_bool(3, speed > 0)
        ...
        if method == 'feed_or_rollback_raw':
            ...
            payload = pb_uint32(1, slot) + pb_uint32(2, speed) + pb_uint32(3, length) + pb_uint32(4, mode)
            return (Cmd.FEED_OR_ROLLBACK, payload)
        if method == 'get_temp':
        ...
        if method == 'set_feed_check':
            check_len = int(params.get('check_length', 73))
            error_len = int(params.get('error_length', 77))
            return (Cmd.SET_FEED_CHECK, pb_uint32(1, check_len) + pb_uint32(2, error_len))
        if method == 'set_fan_raw':
            speed = int(params.get('speed', 0))
            f1 = bool(params.get('fan1', False))
            f2 = bool(params.get('fan2', False))
            return (Cmd.SET_FAN, pb_uint32(1, speed) + pb_bool(2, f1) + pb_bool(3, f2))
        if method == 'drying_raw':
            temp = int(params.get('temp', 50))
            duration = int(params.get('duration', 120))
            auto_roll = bool(params.get('auto_roll', True))
            return (Cmd.DRYING, pb_uint32(1, temp) + pb_uint32(2, duration) + pb_bool(3, auto_roll))
        if method == 'assign_device_id':
            return (Cmd.ASSIGN_DEVICE_ID, pb_uint32(1, int(params.get('uid1', 0))) + pb_uint32(2, int(params.get('uid2', 0))) + pb_uint32(3, int(params.get('uid3', 0))) + pb_uint32(4, int(params.get('device_id', 1))))
        if method == 'raw':
    ...
    def _v2_response_to_v1(self, cmd, seq, payload):
        ...
        if cmd == Cmd.DISCOVER_DEVICE:
            ret['result'] = {'uid1': _fval(fields, 1), 'uid2': _fval(fields, 2), 'uid3': _fval(fields, 3)}
        elif cmd == Cmd.GET_INFO:
            version = _fstr(fields, 1, '')
            boot = _fstr(fields, 2, '')
            ret['result'] = {'model': 'ACE 2 Pro', 'firmware': version, 'boot_version': boot}
        elif cmd == Cmd.GET_STATUS:
            ret['result'] = self._decode_status(fields)
        elif cmd == Cmd.GET_TEMP:
            ret['result'] = {'box1_temp': _fval(fields, 1, 0.0), 'box2_temp': _fval(fields, 2, 0.0), 'ptc1_temp': _fval(fields, 3, 0.0), 'ptc2_temp': _fval(fields, 4, 0.0), 'env_temp': _fval(fields, 5, 0.0), 'env_humidity': _fval(fields, 6, 0.0)}
        elif cmd == Cmd.GET_FILAMENT_INFO:
            sku = _fstr(fields, 3, '')
            ...
            for wtype, color_payload in fields.get(5, []):
                ...
                rgba = _fval(csub, 1, 0)
                color = [rgba >> 24 & 255, rgba >> 16 & 255, rgba >> 8 & 255]
                break
            ret['result'] = {'index': _fval(fields, 1, 0), 'sku': sku, 'type': ftype, 'brand': '', 'color': color, 'rfid': 2 if ftype else 0}
            ...
            for wtype, fi_payload in fields.get(1, []):
                ...
                slots.append({'index': len(slots), 'steps': _fval(fi, 1, 0), 'length': _fval(fi, 2, 0), 'decoder': _fval(fi, 3, 0)})
            ret['result'] = {'feed_info': slots}
        elif cmd == Cmd.GET_KEY_STATE:
            ret['result'] = {'fields': {str(k): _fval(fields, k, 0) for k in fields}}
        else:
    ...
    def _decode_status(self, fields):
        ...
        for wtype, slot_payload in fields.get(9, []):
            ...
            slots.append({'index': len(slots), 'status': filament_state if filament_state != 'identified' else 'ready', 'slot_status': slot_state, 'sku': '', 'type': '', 'rfid': 2 if filament_state == 'identified' else 0, 'brand': '', 'color': [0, 0, 0]})
        while len(slots) < 4:
            slots.append({'index': len(slots), 'status': 'empty', 'slot_status': 'unknown', 'sku': '', 'type': '', 'rfid': 0, 'brand': '', 'color': [0, 0, 0]})
        dry_status = {'status': 'stop', 'target_temp': 0, 'duration': 0, 'remain_time': 0}
        for wtype, dry_payload in fields.get(2, []):
            if wtype != 2:
                continue
            dsub = pb_decode(dry_payload)
            dry_status = {'status': DRY_STATES.get(_fval(dsub, 1, 0), 'stop'), 'target_temp': _fval(dsub, 2, 0), 'duration': _fval(dsub, 3, 0), 'remain_time': _fval(dsub, 4, 0)}
            break
        any_busy = any((s.get('slot_status') in ('feeding', 'rollback', 'preloading') for s in slots))
        return {'status': 'busy' if any_busy else 'ready', 'dryer_status': dry_status, 'temp': _fval(fields, 3, 0), 'humidity': _fval(fields, 4, 0), 'enable_rfid': 1 if _fval(fields, 5, 0) else 0, 'fan_speed': 0, 'feed_assist_count': _fval(fields, 7, 0), 'cont_assist_time': float(_fval(fields, 8, 0)), 'slots': slots}
