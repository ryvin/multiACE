...
#
# Ported from decay71/multiACE 0.98b (tools/merge_ace_cfg.py) under GPL-3.0.
...
line within those sections, if the user's old cfg has the same key with a
different value (or has uncommented a documented key), the user's value
wins. Everything outside [ace]/[ace N] - macros, includes, other sections -
is copied verbatim from the new default.

Multi-line / indented values (e.g. macro bodies) are never touched. A line
is a candidate for substitution only when it starts at column 0 with
`key: value` shape.
...
def parse_user_values(path: str) -> dict[str, dict[str, str]]:
    ...
    Indented lines (multi-line continuations or macro bodies that somehow
    ended up under our section) are ignored."""
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        print('merge_ace_cfg: %s' % e, file=sys.stderr)
        return 1
    except Exception as e:  # noqa: BLE001 - report any parse failure as code 2
