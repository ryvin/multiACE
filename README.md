...
- **Cross-ACE entangle-detect false positive (mitigated v0.82+)** - The stock Snapmaker `filament_entangle_detect` module watches each head's ACE filament-wheel counter and pauses the print with `code:38 detect filament tangled!` if the wheel doesn't tick over ~9mm of extruder motion (threshold = `length × detect_factor × global_sensitivity` = 6 × 1.0 × 1.5 by default). As a consequence of the Cross-ACE feed_assist limitation above, the non-start ACE's wheel never rotates during pull-through extrusion, so the detector trips every ~9mm on heads sourced from non-start ACEs. As of v0.82+, multiACE calls `skip_entangle_check(True)` on the affected detectors at `print_stats:start_printing` and re-enables them on `print_stats:complete/cancelled/error`. The start-ACE head keeps its detector active so real tangles are still caught. Emits audit events `ENTANGLE_SKIP_ENABLED` / `ENTANGLE_SKIP_DISABLED`.
...
- **Manual Heads / TPU (partial)** - A head can be marked "manual" for hand-fed flexible filament (e.g. TPU): set `manual_heads: 0,3` in `[ace]`, or toggle live with `ACE_SET_HEAD_MANUAL HEAD=N ENABLE=0|1` (persists across restarts). A manual head is excluded from ACE automation — `ACE_LOAD_HEAD` / `ACE_UNLOAD_HEAD` refuse it so you load/unload it by hand — and it cannot be flipped while filament is loaded. **Current scope:** the command-level exclusion and persistence only. The mid-print feed-assist / retract / park *bypass* for manual heads (so a hand-fed head is never driven by the ACE during a toolchange) requires correct slot↔head mapping and is pending hardware validation in a follow-up; until then, avoid cross-ACE toolchanges into a manual head mid-print.
