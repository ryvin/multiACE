...
**Live deploy target:** `lava@$DAVINCI_U1_HOST` (root password `snapmaker`). Govee bridge already running on `:7127`. **Print is in progress — do NOT restart Klipper or Moonraker; multiace-web restart is OK.**
...
```bash
curl -s --max-time 5 'http://$DAVINCI_U1_HOST:7125/printer/objects/query?print_stats' | python3 -c "import sys,json; d=json.load(sys.stdin); print('print state:', d['result']['status']['print_stats']['state'])"
```
...
c.connect("$DAVINCI_U1_HOST", username="root", password="snapmaker", timeout=10)
sftp = c.open_sftp()
for f in FILES:
    print("[*] put", f)
...
```bash
sleep 3 && curl -s --max-time 5 'http://$DAVINCI_U1_HOST/multiace/health'
```
Expected: `{"ok":true}`.
...
```bash
curl -s --max-time 5 'http://$DAVINCI_U1_HOST/multiace/api/autodry' | python3 -m json.tool
```
...
```bash
ssh root@$DAVINCI_U1_HOST 'tail -f /home/lava/printer_data/logs/multiace_state.log | grep AUTODRY' &
TAIL_PID=$!
sleep 360
...
```bash
ssh root@$DAVINCI_U1_HOST 'grep "ACE_DRY ACE=" /home/lava/printer_data/logs/klippy.log | tail -5'
```
...
```bash
curl -s --max-time 5 'http://$DAVINCI_U1_HOST/multiace/api/print' | python3 -c "import sys,json; d=json.load(sys.stdin); print('dryer:', d.get('dryer'))"
```
...
```bash
curl -s --max-time 5 'http://$DAVINCI_U1_HOST:7125/server/announcements/list' | python3 -c "import sys,json; d=json.load(sys.stdin); [print(a.get('title'), a.get('description')) for a in d['result']['entries'][:5]]"
```
...
```bash
curl -s --max-time 5 -X POST 'http://$DAVINCI_U1_HOST/multiace/api/autodry' \
...
```bash
ssh root@$DAVINCI_U1_HOST 'grep -E "AUTODRY_TRIGGERED|ACE_DRY ACE=" /home/lava/printer_data/logs/multiace_state.log /home/lava/printer_data/logs/klippy.log | tail -10'
```
...
```bash
ssh root@$DAVINCI_U1_HOST 'grep AUTODRY_FINISHED /home/lava/printer_data/logs/multiace_state.log | tail -3'
```
...
```bash
curl -s 'http://$DAVINCI_U1_HOST:7125/server/announcements/list' | python3 -m json.tool | head -40
```
